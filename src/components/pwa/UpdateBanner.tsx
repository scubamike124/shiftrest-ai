...
export function UpdateBanner() {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed bottom-4 left-1/2 z-[100] -translate-x-1/2 sm:left-auto sm:right-4 sm:translate-x-0"
    >
      <div className="flex items-center gap-3 rounded-full border border-white/15 bg-black/80 px-4 py-2 text-sm text-white shadow-2xl backdrop-blur">
        <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" aria-hidden />
        <span className="font-medium">New version ready</span>
        <button
          type="button"
          onClick={activate}
